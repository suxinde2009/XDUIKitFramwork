...
- (UIView *)makeAccessoryView{
    ...
    if(self.dialogStyle == XDDialogStyleIndeterminate){
        ...
        UIImageView *imageView = [[[UIImageView alloc] initWithImage:UIGraphicsGetImageFromCurrentImageContext()] autorelease];
...
- (void)showOrUpdateAnimatedInternal:(BOOL)flag {
    ...
    // Create overlay
    if (show) {
        self.overlay = overlay = [[[XDDialogWindowOverlay alloc] init] autorelease];
        overlay.opaque = NO;
